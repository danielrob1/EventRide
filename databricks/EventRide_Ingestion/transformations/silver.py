...
# Carga inicial en bulk
def rides_bulk():
    df= spark.readStream.table("bulk_rides")
    df = df.withColumn("booking_timestamp",col("booking_timestamp").cast(T.TimestampType()))
    return df
...
def rides_stream():
    df = spark.readStream.table("eventride.bronze.rides_raw")
